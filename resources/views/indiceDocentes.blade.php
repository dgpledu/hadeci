...
<html lang="en">
  <head>
    ...
    <title>Índice de docentes</title>
  </head>
  <body>
...
<!-- Prueba de borrar tutor -->
          ...
          <form action="/editarDocentes" method="post">
      {{ csrf_field() }}
      <input type="hidden" name="ID_docente_a_editar" value="{{$docente["ID"]}}">
                 {{-- <button class="btn-sm btn-primary btn-xs" type="submit">Editar</button> --}}
           ...
           <form action="" method="post">
              {{ csrf_field() }}
              <input type="hidden" name="ID_docente_a_borrar" value="{{$docente["ID"]}}">
                  {{-- <button class="btn-sm btn-danger btn-xs" type="submit">Borrar</button> --}}
                  <input type="image" img src="/imgs/icono-borrar.png" style="width:30px;height:30px;" onclick="return confirm('¿Seguro que quiere borrar este docente? ATENCIÓN: Se borrará el docente y TODOS los estudiantes registrados por el mismo');">
